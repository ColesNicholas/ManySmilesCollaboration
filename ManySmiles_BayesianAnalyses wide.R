...
lmbf1.1 <- lmBF(happiness ~ condition + image + condition:image + 
                  ResponseId + lab, 
                whichRandom = c("ResponseId", "lab"), rscaleFixed = "wide",
                data = DF.l.inc)
...
write.table(t(round(exp(output), 2)),
            file="outputBayesFactor wide.txt",
            quote = F,
            col.names = F,
            row.names = F,
            append = T)
...
ttestBF(x = DF2.3.1.w$fil.1, 
        y = DF2.3.1.w$happy, 
        paired = T, 
        rscale = sqrt(2)/2) # 5.478931 ±0% but in the oppisite
...
ttestBF(x = DF2.3.2.w$fil.4, 
        y = DF2.3.2.w$happy, 
        paired = T, 
        rscale = sqrt(2)/2) # 0.3721617 ±0.03%%
1/0.3721617 # 2.6

save.image("BayesFactorAnalyses wide.RData")
