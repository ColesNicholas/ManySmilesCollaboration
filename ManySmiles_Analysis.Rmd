...
#### Exploratory primary analyses excluding anxious participants
```{r}
# filter data
tmp <- DF.l.inc %>% 
  filter(anxiety == 1)
...
# descriptives
tmp$ResponseId %>% unique %>% length
tmp %>% 
  group_by(condition) %>%
  summarise(n = unique(ResponseId) %>% length)

tmp2 <- lmer(happiness ~ trial * image * condition +
               (1 | ResponseId) +
               (1 | lab),
             data = DF.l.inc)
