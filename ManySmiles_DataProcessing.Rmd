...
DF %>% 
  filter(lab == "USA_02" |
         ...
         lab == "ESP_01") %>% 
  group_by(lab) %>% 
  summarise(Awareness.M = mean(Awareness),
            Awareness.min = min(Awareness),
            Awareness.max = max(Awareness),
            Distraction.M = mean(Distraction),
...
write_csv(DF.l, "DF.l.csv")
write_csv(DF.l.full, "DF.l.full.csv")
write_csv(DF.w, "DF.w.csv")
```
