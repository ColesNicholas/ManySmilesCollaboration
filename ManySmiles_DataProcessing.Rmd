...
# create list of countries w/ completed open-ended data
...
Center continuous variables that will be used in later moderation analyses
Note: check that this is working correctly
```{r}
DF$qulty_rate_var <- scale(DF$qulty_rate_var,
                           scale = F)

DF$awareness <- scale(DF$awareness,
                      scale = F)
```
...
DF.l <- DF.l %>%
  ...
  select( 
    ...
    awareness, inc.att : inc, # inclusion criteria
