...
DF <- DF %>% 
  ...
  # remove participants who did not get through entire survey
  filter(Progress > 90) %>% 
...
```{r}
DF <- DF %>% 
  mutate(qulty_rate_var = qulty_rate_var - mean(qulty_rate_var,
                                                na.rm = T),
         qulty_smlr_var = qulty_smlr_var - mean(qulty_smlr_var,
                                                na.rm = T),
         qulty_feel_var = qulty_feel_var - mean(qulty_feel_var,
                                                na.rm = T),
         awareness = awareness - mean(awareness,
                                      na.rm = T)
         )
```
...
DF.l <- DF.l %>% 
  ...
  # mean center body awareness variable (for later moderator analyses)
  mutate(indiv_body = indiv_body - mean(indiv_body,
                                        na.rm = T))
```
...
```{r}
# dataset containing filler and pose trials
DF.l.full <- DF.l

# dataset containing only pose trials
DF.l <- DF.l %>%
  filter(trial == "happy" |
         trial == "neutr") %>% 
  mutate(trial = factor(trial)) # relevel trial factor

# dataset containing pose trials only from participants 
# who meet strict inclusion criteria
DF.l.inc <- DF.l %>% 
  filter(inc == 1,
         inc.awa == 1)
```
...
write_rds(DF.l.inc, "data/DF.l.inc.rds")
