# -----------------
# Prep environment
# -----------------
# clear environment
rm(list = ls())

# set seed
set.seed(2)

# turn scientific notation off
options(scipen = 999)

# load results
# load("BayesFactorAnalyses_maximal.RData")

# allow for Type III SS
options(contrasts = c('contr.sum', 'contr.poly'))

# load packages
library('tidyverse')
library('BayesFactor')

# load data
DF.l.inc <- readRDS("data/processed/DF.l.inc.rds") %>% 
  mutate(ResponseId = ResponseId %>% 
           as.numeric() %>% 
           as.factor())

#################
# a  # Image effect
#################
lmbfa.1 <- lmBF(happiness ~ image +
                  ResponseId + lab + lab:image, 
                whichRandom = c("ResponseId", "lab", "lab:image"), 
                data = DF.l.inc)


lmbfa.0 <- lmBF(happiness ~ 1  +
                  ResponseId + lab + lab:image, 
                whichRandom = c("ResponseId", "lab", "lab:image"), 
                data = DF.l.inc)

(bf2 <- lmbfa.1/lmbfa.0 )


#################
# 1  #
#################
# Interaction between Facial Movement Task and Stimuli Presence
lmbf1.1 <- lmBF(happiness ~ condition + image 
                + condition:image + 
                  ResponseId + lab + lab:condition + lab:image + lab:condition:image, 
                whichRandom = c("ResponseId", "lab", "lab:condition", "lab:image", "lab:condition:image"), 
                data = DF.l.inc)


lmbf1.0 <- lmBF(happiness ~ condition + image + 
                ResponseId + lab + lab:condition + lab:image + lab:condition:image, 
                whichRandom = c("ResponseId", "lab", "lab:condition", "lab:image", "lab:condition:image"), 
                data = DF.l.inc)

(bf1 <- lmbf1.1/lmbf1.0 ) #  0.2167649 ±20.15% (Evidence for the null 4.63 ±20.15%) OLD: in conclusive: 0.8633591 ?15.52%

#################
# 2  #
#################
# Three-way interaction between Pose, Facial Movement Task, and Stimuli Presence
lmbf2.1 <- lmBF(happiness ~ condition + image + trial + 
                  condition : image + condition : trial + image : trial 
                + condition : image : trial +  
                  ResponseId + lab + lab:condition + lab:image + lab:trial + lab:condition:image, 
                whichRandom = c("ResponseId", "lab", 
                                "lab:condition", "lab:image", "lab:trial", 
                                 "lab:condition:image","lab:condition:trial", "lab:image:trial",
                                "lab:condition:image:trial"), 
                data = DF.l.inc)

lmbf2.0 <- lmBF(happiness ~ condition + image + trial + 
                  condition : image + condition : trial + 
                  image : trial +  
                  ResponseId + lab + lab:condition + lab:image + lab:trial + lab:condition:image, 
                whichRandom = c("ResponseId", "lab", 
                                "lab:condition", "lab:image", "lab:trial", 
                                "lab:condition:image","lab:condition:trial", "lab:image:trial",
                                "lab:condition:image:trial"), 
                data = DF.l.inc)

(bf2 <- lmbf2.1/lmbf2.0 )# 1.952202 ±20.35% inconclusive. OLD: 1.678069 ±11.69%

# Simple effects:

#Mimicry
#################
# 3  #
#################
# Effect of Stimuli Presence

lmbf3.1 <- lmBF(happiness ~ image +  
                  ResponseId + lab 
                + lab:image,
                whichRandom = c("ResponseId", "lab", 
                                "lab:image"), 
                data = subset(DF.l.inc, 
                              condition == "mimicry"))

lmbf3.0 <- lmBF(happiness ~ 1 +
                  ResponseId + lab + 
                lab:image,
                whichRandom = c("ResponseId", "lab", 
                                "lab:image"),
                data = subset(DF.l.inc, 
                              condition == "mimicry"))

(bf3 <- lmbf3.1/lmbf3.0) # 1.838355 ±6.73%, OLD: 3.763

#################
# 4  #
#################
# Effect of Pose

lmbf4.1 <- lmBF(happiness ~ trial +  
                  ResponseId + lab + 
                    lab:trial,
                whichRandom = c("ResponseId", "lab", 
                                "lab:trial"), 
                data = subset(DF.l.inc, 
                              condition == "mimicry"))

lmbf4.0 <- lmBF(happiness ~ 1 +  
                  ResponseId + lab+ 
                    lab:trial,
                whichRandom = c("ResponseId", "lab", 
                                "lab:trial"), 
                data = subset(DF.l.inc, 
                              condition == "mimicry"))

(bf4 <- lmbf4.1/lmbf4.0) # 852.6349 ±3.05%. OLD: 25237385930862  ±2.85%

#################
# 5  #
#################
# Interaction between Pose and Stimuli Presence

lmbf5.1 <- lmBF(happiness ~ image + trial + image : trial +
                  ResponseId + lab +
                    lab:image + lab:trial +
                    lab:image:trial,
                whichRandom = c("ResponseId", "lab",
                                "lab:image", "lab:trial", "lab:image:trial"), 
                data = subset(DF.l.inc, 
                              condition == "mimicry"))

lmbf5.0 <- lmBF(happiness ~ image + trial +
                  ResponseId + lab +
                    lab:image + lab:trial +
                    lab:image:trial,
                whichRandom = c("ResponseId", "lab",
                                "lab:image", "lab:trial", "lab:image:trial"), 
                data = subset(DF.l.inc, 
                              condition == "mimicry"))

(bf5 <- lmbf5.1/lmbf5.0) # 0.4168796 ±8.18% OLD: 2.82

# Mimicry and  present
#################
# 6  #
#################
# The effect of Pose emerged in the presence

lmbf6.1 <- lmBF(happiness ~ trial + 
                  ResponseId + lab +
                    lab:trial, 
                whichRandom = c("ResponseId", "lab", "lab:trial"), 
                data = subset(DF.l.inc, 
                              condition == "mimicry" & image == "present"))

lmbf6.0 <- lmBF(happiness ~ 1 + 
                  ResponseId + lab +
                    lab:trial, 
                whichRandom = c("ResponseId", "lab", "lab:trial"), 
                data = subset(DF.l.inc, 
                              condition == "mimicry" & image == "present"))

(bf6 <- lmbf6.1/lmbf6.0 )# 6.475089 ±12.79% OLD: 810.0388 ± 6.03%

# Mimicry and  absentt
#################
# 7  #
#################
# The effect of Pose emerged in the in the absence

lmbf7.1 <- lmBF(happiness ~ trial + 
                  ResponseId + lab +
                    lab:trial, 
                whichRandom = c("ResponseId", "lab", "lab:trial"), 
                data = subset(DF.l.inc, 
                              condition == "mimicry" & image == "absentt"))

lmbf7.0 <- lmBF(happiness ~ 1 +  
                  ResponseId + lab +
                    lab:trial, 
                whichRandom = c("ResponseId", "lab", "lab:trial"), 
                data = subset(DF.l.inc, 
                              condition == "mimicry" & image == "absentt"))

(bf7 <-lmbf7.1/lmbf7.0) # 981.9743 ±4.76% OLD: 20327954614


# Directed
#################
# 8  #
#################
# Effect of Stimuli Presence

lmbf8.1 <- lmBF(happiness ~ image +
                  ResponseId + lab +
                    lab:trial, 
                whichRandom = c("ResponseId", "lab", "lab:image"), 
                data = subset(DF.l.inc, 
                              condition == "directd"))

lmbf8.0 <- lmBF(happiness ~ 1 + 
                  ResponseId + lab +
                    lab:trial, 
                whichRandom = c("ResponseId", "lab", "lab:image"), 
                data = subset(DF.l.inc, 
                              condition == "directd"))

(bf8 <- lmbf8.1/lmbf8.0 )# 123.8126 ±4.84% OLD: 113.534

#################
# 9  #
#################
# Effect of Pose

lmbf9.1 <- lmBF(happiness ~ trial +
                  ResponseId + lab +
                    lab:trial, 
                whichRandom = c("ResponseId", "lab", 
                                "lab:trial"), 
                data = subset(DF.l.inc, 
                              condition == "directd"))

lmbf9.0 <- lmBF(happiness ~ 1 + 
                  ResponseId + lab +
                    lab:trial, 
                whichRandom = c("ResponseId", "lab", 
                                "lab:trial"), 
                data = subset(DF.l.inc, 
                              condition == "directd"))

(bf9 <- lmbf9.1/lmbf9.0) # 37.31523 ±3.25% OLD: 1871459923 ± 3.33%


#################
# 10  #
#################
# Interaction between Pose and Stimuli Presence

lmbf10.1 <-  lmBF(happiness ~ image + trial + image : trial +  
                    ResponseId + lab +
                      lab:image + lab:trial + 
                      lab:image:trial, 
                  whichRandom = c("ResponseId", "lab", 
                                  "lab:image", "lab:trial", 
                                  "lab:image:trial"), 
                  data = subset(DF.l.inc, 
                                condition == "directd"))

lmbf10.0 <-  lmBF(happiness ~ image + trial +  
                    ResponseId + lab +
                      lab:image + lab:trial + 
                      lab:image:trial, 
                  whichRandom = c("ResponseId", "lab", 
                                  "lab:image", "lab:trial", 
                                  "lab:image:trial"), 
                  data = subset(DF.l.inc, 
                                condition == "directd"))

(bf10 <- lmbf10.1/lmbf10.0) # 0.1785788 ±6.61% OLD 0.1043555 ±3.37%
1/bf10 # 5.59977 ±6.61% OLD: 9.58

# directd and  present
#################
# 11  #
#################
# The effect of Pose  in the presence

lmbf11.1 <- lmBF(happiness ~ trial +
                   ResponseId + lab + 
                     lab:trial, 
                 whichRandom = c("ResponseId", "lab", 
                                 "lab:trial"), 
                 data = subset(DF.l.inc, 
                               condition == "directd" & image == "present"))

lmbf11.0 <- lmBF(happiness ~ 1 +
                   ResponseId + lab + 
                     lab:trial, 
                 whichRandom = c("ResponseId", "lab", 
                                 "lab:trial"), 
                 data = subset(DF.l.inc, 
                               condition == "directd" & image == "present"))

(bf11 <- lmbf11.1/lmbf11.0) # 1.774162 ±4.92% OLD: 895.4185 ±1.21%

# directd and  absentt
#################
# 12  #
#################
# The effect of Pose  in the absence

lmbf12.1 <- lmBF(happiness ~ trial + 
                   ResponseId + lab 
                 + lab:trial, 
                 whichRandom = c("ResponseId", "lab", 
                                 "lab:trial"), 
                 data = subset(DF.l.inc, 
                               condition == "directd" & image == "absentt"))

lmbf12.0 <- lmBF(happiness ~ 1 + 
                   ResponseId + lab + 
                     lab:trial, 
                 whichRandom = c("ResponseId", "lab", 
                                 "lab:trial"), 
                 data = subset(DF.l.inc, 
                               condition == "directd" & image == "absentt"))

(bf12 <- lmbf12.1/lmbf12.0) # 54.28988 ±4.3% OLD: 578494.6 ±3.33%

# Pen-in-the-mouth

#################
# 13  #
#################
# Effect of Stimuli Presence
lmbf13.1 <- lmBF(happiness ~ image +
                   ResponseId + lab +
                     lab:image, 
                 whichRandom = c("ResponseId", "lab", 
                                 "lab:image"), 
                 data = subset(DF.l.inc, 
                               condition == "pentask"))

lmbf13.0 <- lmBF(happiness ~ 1 +
                   ResponseId + lab + 
                     lab:image, 
                 whichRandom = c("ResponseId", "lab", 
                                 "lab:image"), 
                 data = subset(DF.l.inc, 
                               condition == "pentask"))

(bf13 <- lmbf13.1/lmbf13.0 )#  0.3561945 ±16.66% OLD: 0.2058395
1/bf13 # 2.807455 ±16.66% ±16.66% OLD: 4.858154

#################
# 14  #
#################
# Effect of Pose

lmbf14.1 <- lmBF(happiness ~ trial +
                   ResponseId + lab + lab:trial, 
                 whichRandom = c("ResponseId", "lab", "lab:trial"), 
                 data = subset(DF.l.inc, 
                               condition == "pentask"))

lmbf14.0 <- lmBF(happiness ~ 1 +
                   ResponseId + lab + lab:trial, 
                 whichRandom = c("ResponseId", "lab", "lab:trial"), 
                 data = subset(DF.l.inc, 
                               condition == "pentask"))

(bf14 <- lmbf14.1/lmbf14.0) # 0.1649866 ±11.28% OLD: 1.001017
1/bf14 # 6.061097 ±11.28%

#################
# 15  #
#################
# Interaction between Pose and Stimuli Presence
lmbf15.1 <-  lmBF(happiness ~ image + trial + image : trial +
                    ResponseId + lab + 
                      lab:image + lab:trial + 
                      lab:image:trial, 
                  whichRandom = c("ResponseId", "lab", 
                                  "lab:image", "lab:trial", 
                                  "lab:image:trial"), 
                  data = subset(DF.l.inc, 
                                condition == "pentask"))

lmbf15.0 <-  lmBF(happiness ~ image + trial + 
                    ResponseId + lab + 
                      lab:image + lab:trial + 
                      lab:image:trial, 
                  whichRandom = c("ResponseId", "lab", 
                                  "lab:image", "lab:trial", 
                                  "lab:image:trial"), 
                  data = subset(DF.l.inc, 
                                condition == "pentask"))

(bf15 <- lmbf15.1/lmbf15.0) #  4.629564 ±116.8% OLD:0.196295 
1/bf15

# pentask and  present
#################
# 16  #
#################
# The effect of Pose in the presence
lmbf16.1 <- lmBF(happiness ~ trial +  
                   ResponseId + lab + 
                     lab:trial, 
                 whichRandom = c("ResponseId", "lab", "lab:trial"), 
                 data = subset(DF.l.inc, 
                               condition == "pentask" & image == "present"))

lmb16.0 <- lmBF(happiness ~ 1 + 
                  ResponseId + lab + 
                    lab:trial, 
                whichRandom = c("ResponseId", "lab", "lab:trial"), 
                data = subset(DF.l.inc, 
                              condition == "pentask" & image == "present"))

(bf16 <- lmbf16.1/lmb16.0) # 0.5452515 ±30.49% OLD: 2.5
1/bf16 #1.834016 ±30.49%

# pentask and  absentt
#################
# 17  #
#################
# The effect of Pose  in the the absence
lmbf17.1 <- lmBF(happiness ~ trial +
                   ResponseId + lab + 
                     lab:trial, 
                 whichRandom = c("ResponseId", "lab", 
                                 "lab:trial"), 
                 data = subset(DF.l.inc, 
                               condition == "pentask" & image == "absentt"))

lmbf17.0 <- lmBF(happiness ~ 1 +
                   ResponseId + lab + 
                     lab:trial, 
                 whichRandom = c("ResponseId", "lab", 
                                 "lab:trial"), 
                 data = subset(DF.l.inc, 
                               condition == "pentask" & image == "absentt"))

(bf17 <- lmbf17.1/lmbf17.0) # 0.2852611 ±44.11% OLD: 0.1222945 ±12.31%
1/bf17 # 3.50556 ±44.11% OLD: 8.176982 ±12.31%

#################
# pen-in-mouth with relaxed exclusion criteria  #
#################
# load data
DF.l <- readRDS("data/processed/DF.l.rds") %>% #  old DF.incl 
  mutate(ResponseId = ResponseId %>% 
           as.numeric() %>% 
           as.factor())

lmbf18.1 <- lmBF(happiness ~ trial +  
                   ResponseId + lab + 
                     lab:trial, 
                 whichRandom = c("ResponseId", "lab", 
                                 "lab:trial"), 
                 data = subset(DF.l, 
                               condition == "pentask"))

lmbf18.0 <- lmBF(happiness ~ 1 +
                   ResponseId + lab + 
                     lab:trial, 
                 whichRandom = c("ResponseId", "lab", 
                                 "lab:trial"), 
                 data = subset(DF.l, 
                               condition == "pentask"))

(bf18 <- lmbf18.1/lmbf18.0) #   0.9470488 ±1.95% OLD: 277289.3 ±7.34%

#################
# comparisons with filler tasks when images are absent  #
#################
# load data
DF.l.full.inc <- readRDS("data/processed/DF.l.full.inc.rds") %>% #  old DF.full 
  mutate(ResponseId = ResponseId %>% 
           as.numeric() %>% 
           as.factor())

lmbf19.1 <- lmBF(happiness ~ condition * trial +
                   ResponseId + lab + 
                     lab:trial + lab:condition + 
                     lab:condition:trial, 
                 whichRandom = c("ResponseId", "lab",
                                 "lab:trial", "lab:condition",
                                 "lab:condition:trial"), 
                 data = subset(DF.l.full.inc, 
                               image == "absentt"))

lmbf19.0 <- lmBF(happiness ~ condition + trial +  
                   ResponseId + lab + 
                     lab:trial + lab:condition + 
                     lab:condition:trial, 
                 whichRandom = c("ResponseId", "lab",
                                 "lab:trial", "lab:condition",
                                 "lab:condition:trial"), 
                 data = subset(DF.l.full.inc, 
                               image == "absentt"))

(bf19 <- lmbf19.1/lmbf19.0) # 0.0001137237 ±140.88% OLD: 120809211773 ±7.75%

output = c(bf1@bayesFactor$bf, bf2@bayesFactor$bf, 
           bf3@bayesFactor$bf, bf4@bayesFactor$bf, 
           bf5@bayesFactor$bf, bf6@bayesFactor$bf, 
           bf7@bayesFactor$bf, bf8@bayesFactor$bf, 
           bf9@bayesFactor$bf, bf10@bayesFactor$bf, 
           bf11@bayesFactor$bf, bf12@bayesFactor$bf, 
           bf13@bayesFactor$bf, bf14@bayesFactor$bf, 
           bf15@bayesFactor$bf, bf16@bayesFactor$bf, 
           bf17@bayesFactor$bf, bf18@bayesFactor$bf, 
           bf19@bayesFactor$bf)

write.table(t(round(exp(output), 2)),
            file="outputBayesFactor.txt",
            quote = F,
            col.names = F,
            row.names = F,
            append = T)

library(tidyr)
DF2.1.1 <- subset(DF.l.full.inc, 
                  image == "absentt" & 
                    condition == "mimicry" & 
                    (trial == "fil.1" |trial == "happy"))

DF2.1.1 <- droplevels(DF2.1.1)

DF2.1.1 <- DF2.1.1[, c('ResponseId', 'happiness', 'trial')]

DF2.1.1.w <- data.frame(spread(DF2.1.1, 
                               key = trial, 
                               value = happiness))

ttestBF(x = DF2.1.1.w$fil.1, 
        y = DF2.1.1.w$happy, 
        paired = T, 
        rscale = 0.5) # 3011970 ±0%

DF2.1.2 <- subset(DF.l.full.inc, 
                  image == "absentt" & 
                    condition == "mimicry" & 
                    (trial == "fil.4" | trial == "happy"))

DF2.1.2 <- droplevels(DF2.1.2)

DF2.1.2 <- DF2.1.2[, c('ResponseId', 'happiness', 'trial')]

DF2.1.2.w <- data.frame(spread(DF2.1.2, 
                               key = trial, 
                               value = happiness))

ttestBF(x = DF2.1.2.w$fil.4, 
        y = DF2.1.2.w$happy, 
        paired = T, 
        rscale = 0.5) # 9364341 ±0%%


DF2.2.1 <- subset(DF.l.full.inc, 
                  image == "absentt" & 
                    condition == "directd" & 
                    (trial == "fil.1" | trial == "happy"))

DF2.2.1 <- droplevels(DF2.2.1)

DF2.2.1 <- DF2.2.1[, c('ResponseId', 'happiness', 'trial')]

DF2.2.1.w <- data.frame(spread(DF2.2.1, 
                               key = trial, 
                               value = happiness))

ttestBF(x = DF2.2.1.w$fil.1, 
        y = DF2.2.1.w$happy, 
        paired = T, 
        rscale = 0.5) # 3.542896 ±0%%

DF2.2.2 <- subset(DF.l.full.inc, 
                  image == "absentt" & 
                    condition == "directd" & 
                    (trial == "fil.4" | trial == "happy"))

DF2.2.2 <- droplevels(DF2.2.2)

DF2.2.2 <- DF2.2.2[, c('ResponseId', 'happiness', 'trial')]

DF2.2.2.w <- data.frame(spread(DF2.2.2, 
                               key = trial, 
                               value = happiness))

ttestBF(x = DF2.2.2.w$fil.4, 
        y = DF2.2.2.w$happy, 
        paired = T, 
        rscale = 0.5) # 7.412424 ±0%

DF2.3.1 <- subset(DF.l.full.inc, 
                  image == "absentt" & 
                    condition == "pentask" & 
                    (trial == "fil.1" | trial == "happy"))

DF2.3.1 <- droplevels(DF2.3.1)

DF2.3.1 <- DF2.3.1[, c('ResponseId', 'happiness', 'trial')]

DF2.3.1.w <- data.frame(spread(DF2.3.1, 
                               key = trial, 
                               value = happiness))

ttestBF(x = DF2.3.1.w$fil.1, 
        y = DF2.3.1.w$happy, 
        paired = T, 
        rscale = 0.5) # 5.478931 ±0% but in the oppisite

summary(DF2.3.1.w)

DF2.3.2 <- subset(DF.l.full.inc, 
                  image == "absentt" & 
                    condition == "pentask" & 
                    (trial == "fil.4" | trial == "happy"))

DF2.3.2 <- droplevels(DF2.3.2)

DF2.3.2 <- DF2.3.2[, c('ResponseId', 'happiness', 'trial')]

DF2.3.2.w <- data.frame(spread(DF2.3.2, 
                               key = trial, 
                               value = happiness))

ttestBF(x = DF2.3.2.w$fil.4, 
        y = DF2.3.2.w$happy, 
        paired = T, 
        rscale = 0.5) # 0.3721617 ±0.03%%
1/exp(ttestBF(x = DF2.3.2.w$fil.4, 
          y = DF2.3.2.w$happy, 
          paired = T, 
          rscale = 0.5)@bayesFactor$bf) # 2.69

save.image("BayesFactorAnalyses_maximal.RData")

